...
export default function PublicPage() {
	...
	// Date-based permalinks (/%year%/%monthnum%/%postname%/) put the post
	// slug in the last segment; pages match on their full path.
	const postSlug = slugParts[slugParts.length - 1] ?? '';

	const { data: postsData, isLoading } = useQuery({
		queryKey: ['public-page', slug],
		queryFn: async () => {
			const pages = await api.get<{ posts: Post[] }>(
				`/posts?slug=${encodeURIComponent(slug)}&type=page&status=publish`,
			);
			if (pages.posts.length > 0) return pages.posts[0];
			const posts = await api.get<{ posts: Post[] }>(
				`/posts?slug=${encodeURIComponent(postSlug)}&type=post&status=publish`,
			);
	...
	return (
		<article style={{ maxWidth: 800, margin: '0 auto', padding: '32px 24px' }}>
			<h1 style={{ fontSize: '2em', fontWeight: 700, marginBottom: 16 }}>{postsData.postTitle}</h1>
			<div
				style={{ lineHeight: 1.8 }}
				dangerouslySetInnerHTML={{ __html: postsData.postContent }}
			/>
		</article>
	);
}
...
function buildKitCSS(kit: KitData): string {
	...
	if (kit.typography)
		for (const t of kit.typography) {
			const ff = (t as Record<string, unknown>).fontFamily;
			if (typeof ff === 'string') fonts.add(ff);
		}
	const imp =
		fonts.size > 0
